...
public class SourceCodePruner
{
     ...
     * @param dstProject
     */
    public void pruneFeatures(SymbolTable sbTable, String dstProject, FileFilter filter)
    {
		...
		if (filter == null) {
			...
			ProcessorUtils.relocate(srcDir, dstDir, filter);						
		}
    ...
    private void pruneSrcFile(File srcFile, List<String> selectedFeatures)
    {
        ...
        try {
        	...
        	final String srcCode = srcPruner.produce();
        	if (srcCode.isEmpty()) {
        		srcFile.delete();
        	} else {
